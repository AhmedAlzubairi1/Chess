...
class Piece():
    def __init__(self, board, color, group, name, row, col, game):
        ...
        assert col in self.possibleCol, "Invalid column location, make sure it is between A and H (inclusive and uppercase)"
        assert row in self.possibleRow, "Invalid row location, make sure it is between 1 and 8 (inclusive)"
        assert color in (
            "BLACK", "WHITE"), "Invalid color for piece, make it it is either BLACK or WHITE"
        self.board = board
        self.color = color
        self.group = group
        self.name = name
    ...
    def notKing(self, row, col):
        ...
        :rtype: bool
        """
        # TODO this should check if the location is a king or near a king. If
        # it is, return False. Else return True
    ...
    def __eq__(self, other):
        ...
        return isinstance(other, type(
            self)) and self.name == other.name and self.origCol == other.origCol and self.origRow == other.origRow

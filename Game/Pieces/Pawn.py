...
class Pawn(Piece):
    ...
    def availableMoves(self):
        ...
        if self.color == 'BLACK':
            # top position
            if 0 <= tempRow + \
                    1 <= 7 and self.board[tempRow + 1][tempCol] is None:
                moves.add((tempRow + 1, tempCol))

            # if at second col
            if tempRow == 1 and self.board[tempRow +
                                           1][tempCol] is None and self.board[tempRow +
                                                                              2][tempCol] is None:
            ...
            if 0 <= tempRow + 1 <= 7 and 0 <= tempCol - \
                    1 <= 7 and self.board[tempRow + 1][tempCol - 1] is not None and self.validMove(tempRow + 1, tempCol - 1):
            ...
            if 0 <= tempRow + 1 <= 7 and 0 <= tempCol - \
                    1 <= 7 and self.board[tempRow + 1][tempCol - 1] is None and self.board[tempRow][tempCol - 1] in self.game.playerTwoPassantPawns:
            ...
            # if at second col
            if tempRow == 6 and self.board[tempRow -
                                           1][tempCol] is None and self.board[tempRow -
                                                                              2][tempCol] is None:
                moves.add((tempRow - 2, tempCol))
            ...
            if 0 <= tempRow - 1 <= 7 and 0 <= tempCol - \
                    1 <= 7 and self.board[tempRow - 1][tempCol - 1] is None and self.board[tempRow][tempCol - 1] in self.game.playerOnePassantPawns:
                moves.add((tempRow - 1, tempCol - 1))
    ...
    def isPassant(self, tempOriginalRow, targetRow, targetCol):
        ...
        if not self.hasMoved and self.col == targetCol and abs(
                tempOriginalRow - targetRow) == 2:
            return True
        return False
    ...
    def move(self, targetRow, targetCol):
        ...
        preMoveRow = self.row
        if (targetRow - 1,
                self.possibleCol[targetCol]) in self.availableMoves():
            # Check if there is already a piece there. If so, remove it and
            # replace
            if self.board[targetRow -
                          1][self.possibleCol[targetCol]] is not None:
                # print("if")
                self.board[targetRow -
                           1][self.possibleCol[targetCol]].removeFromGroup()
                ...
                # Note: I need to know check of the case it is en passant
                # capture, not just a regular move
                self.board[targetRow - 1][self.possibleCol[targetCol]] = self
                self.board[self.row - 1][self.possibleCol[self.col]] = None
                self.row = targetRow
                self.col = targetCol
                # Check if you can do an en passant capture on oppoenent piece
                if self.color == 'BLACK' and self.board[targetRow -
                                                        2][self.possibleCol[targetCol]] in self.game.playerTwoPassantPawns:
                    self.board[targetRow -
                               2][self.possibleCol[targetCol]].removeFromGroup()
                    self.board[targetRow -
                               2][self.possibleCol[targetCol]] = None
                    ...
                    self.board[targetRow][self.possibleCol[targetCol]
                                          ].removeFromGroup()
            ...
            # Here I check if it is a candidate for a passant move first before
            # I label hasMoved as true
    ...
    def possibleCapturesCheck(self):
        ...
        if self.color == 'BLACK':
            # Now the capture moves
            if 0 <= tempRow + 1 <= 7 and 0 <= tempCol - \
                    1 <= 7 and self.board[tempRow + 1][tempCol - 1] is not None and self.validMoveIncludingKing(tempRow + 1, tempCol - 1):
                moves.add((tempRow + 1, tempCol - 1))
